...
distribution shift that degrades learning quality, and that
this effect is a primary driver of on-policy PPO's superiority in safety-critical
autonomous driving.}
...
outperforming our 5-component Rainbow DQN (88\% success, 12\% collision) and dramatically
outperforming vanilla DQN (20\% success, 80\% collision).
...
\begin{enumerate}
  \item \textbf{A falsifiable hypothesis and empirical test.} We frame the on-policy
        ...
        shift, not architectural differences, is a primary cause of PPO's advantage
        in dense traffic (50~IDM vehicles, fixed density). We provide statistical
        evidence via Wilcoxon signed-rank tests ($p < 0.05$) and Cohen's $d$ effect
        sizes across all algorithm pairs.
  \item \textbf{A complete, fair benchmark.} Five DRL algorithms share identical
        ...
        SB3-compatible 5-component Rainbow DQN with PER, NoisyNets, and $n$-step returns.
...
Table~\ref{tab:results} presents the full evaluation results.
Throughout, ``Rainbow (5/6)'' refers to our 5-component implementation
(Section~IV-D, excluding C51). The results reveal a clear
two-tier separation: PPO and Rainbow (5/6) form a high-performing group, while
Dueling DQN, Double DQN, and DQN exhibit dramatically higher collision rates.

\begin{table}[htbp]
\centering
\caption{Evaluation Results (50 Episodes, seed 42). \textbf{Bold} = best.}
\label{tab:results}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Rainbow (5/6)      & 88\%             & 29.21 $\pm$ 6.95     & 20.67               & 12\%  \\
...
The primary result — that on-policy PPO decisively outperforms our heavily-engineered
off-policy 5-component Rainbow DQN — is counter-intuitive. The original
Rainbow~\cite{hessel2018rainbow} was specifically designed as
the ``best of all worlds'' for value-based RL, and yet PPO
surpasses our implementation by 8 percentage points in success rate and nearly halves
the collision rate. We identify three complementary mechanisms explaining this finding.
...
This does not mean off-policy methods are inherently inferior — our results are
obtained at a single, high density (50~vehicles). We hypothesise that in low-density
traffic (fewer surrounding vehicles, rarer crashes) the distribution shift problem
is less severe and Rainbow's sample efficiency advantage may dominate.
A controlled density sweep is needed to test this and is planned for future work.
...
highway driving in dense, multi-agent traffic (50~IDM vehicles, seed~42).
Our central finding is that PPO
achieves 96\% success rate with only 4\% collisions, outperforming our 5-component
...
problem domains. The original Rainbow DQN was engineered to excel on Atari — a domain
where safety is irrelevant and exploration cost is low. In safety-critical autonomous
driving, on-policy learning's immunity to distribution shift appears to be a more
valuable property than any individual DQN enhancement, though this conclusion
requires validation across multiple seeds and traffic densities.
We hope these findings guide practitioners toward more
principled algorithm selection for safety-critical RL applications.
